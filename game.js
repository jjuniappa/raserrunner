(() => {
  ...
  const CONFIG = {
    ...
    // 원본 PNG의 캔버스 크기와 무관하게 화면상의 캐릭터 크기를 고정한다.
    // 점프 에셋을 크롭해 높이가 달라져도 캐릭터가 갑자기 작아지거나 커지지 않는다.
    playerRunDrawHeight: 428,
    playerJumpDrawHeight: 428,
    playerSlideDrawHeight: 205,
  ...
  function getJumpOffsetY() {
    if (state.player.action !== "jump") return 0;
    const t = Math.min(1, state.player.actionTime / CONFIG.jumpDuration);
    return -Math.sin(Math.PI * t) * CONFIG.jumpHeight;
  }
  ...
  function getPlayerVisual() {
    ...
    if (state.player.action === "jump") {
      frames = assets.jump;
      drawHeight = CONFIG.playerJumpDrawHeight;
      const t = Math.min(1, state.player.actionTime / CONFIG.jumpDuration);
      frameIndex = Math.min(frames.length - 1, Math.floor(t * frames.length));
    } else if (state.player.action === "slide") {
      frames = assets.slide;
      drawHeight = CONFIG.playerSlideDrawHeight;
      const t = Math.min(1, state.player.actionTime / CONFIG.slideDuration);
      frameIndex = Math.min(frames.length - 1, Math.floor(t * frames.length));
    } else {
      frames = assets.run;
      drawHeight = CONFIG.playerRunDrawHeight;
      frameIndex = Math.floor(state.player.animTime * CONFIG.runFps) % frames.length;
    }

    const image = frames[frameIndex];
    const width = drawHeight * (image.width / image.height);
    const footY = CONFIG.playerGroundY + getJumpOffsetY();
  ...
  function getPlayerHitbox() {
    ...
    // 히트박스도 화면에 그려지는 좌표를 그대로 따라간다.
    // 새 점프 에셋은 투명 여백을 제거하고 발바닥을 캔버스 하단에 맞추는 것을 전제로 한다.
    if (state.player.action === "jump") {
      return {
        x: r.x + r.width * 0.22,
        y: r.y + r.height * 0.12,
        width: r.width * 0.56,
        height: r.height * 0.82
      };
    }

    if (state.player.action === "slide") {
      return {
        x: r.x + r.width * 0.08,
        y: r.y + r.height * 0.12,
        width: r.width * 0.84,
  ...
  function update(dt) {
    ...
    for (const obstacle of state.obstacles) {
      const previousProgress = obstacle.progress;
      obstacle.progress += speed * dt;

      // 레이저가 플레이어 지점을 실제로 통과하는 프레임에서 한 번만 판정한다.
      // 넓은 구간에서 반복 판정해 점프 초반에 먼저 맞는 현상을 방지한다.
      const crossedPlayer =
        previousProgress < CONFIG.collisionProgress &&
        obstacle.progress >= CONFIG.collisionProgress;
