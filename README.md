...
## v2 레이저 높이 수정
- 낮은 레이저는 바닥 바로 위에 표시됩니다.
- 높은 레이저는 캐릭터 상체보다 훨씬 위쪽에 표시됩니다.
- 접근 중에는 JUMP 또는 SLIDE 안내가 표시됩니다.


## v3 실제 히트박스 충돌 판정

- 캐릭터 블록의 현재 화면 크기와 위치를 기준으로 히트박스를 계산합니다.
- 점프 중에는 캐릭터 히트박스가 실제로 위로 이동합니다.
- 슬라이딩 중에는 히트박스가 실제로 낮고 넓게 변합니다.
- 레이저 선의 위치와 두께가 캐릭터 히트박스와 겹칠 때만 게임오버가 됩니다.
- 서비스 워커 캐시 버전을 갱신해 이전 코드가 남는 문제를 줄였습니다.
